...
.PHONY: help lint secrets style hooks test test-unit test-integration install-cli uninstall-cli packages iso docs docs-serve clean version
...
lint: secrets style ## Vérifier les scripts Bash avec shellcheck, l'absence de secrets et le style
...
style: ## Vérifier le style des textes (pas de tirets cadratins)
